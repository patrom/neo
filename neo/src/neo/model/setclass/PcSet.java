...
public class PcSet extends Set {
	...
	public String removeConsecutiveSpaces(String s) {
		...
		StringBuffer noConsecutiveSpacesStringBuffer 
		    = new StringBuffer();
		int ii = 0;
	...
	public String removeDuplicatesInString(String s) {
		...
		StringBuffer withoutDuplicatesStringBuffer 
		    = new StringBuffer();
		for (int i = 0; i < newSize; i++)
			withoutDuplicatesStringBuffer.append(pcs2[i]);

		String withoutDuplicatesString 
		    = withoutDuplicatesStringBuffer.toString();
	...
	while (intervals[s.length - numberOfLargestIntervals]
	       == intervals[s.length - (numberOfLargestIntervals + 1)] 
	...
	if (numberOfLargestIntervals == 1) {
		...
		for(int ii = 0; ii < s.length; ii++)
		    next[i][ii]
		      = s[(intervalOrder[s.length - (1 + i)] + ii) % s.length];
		...
		do {
		    if(
		     ((next[smallest][ii + 1] + 12) - next[smallest][ii]) % 12 
		       > ((next[i][ii + 1] + 12) - next[i][ii]) % 12) {
			smallest = i;
			break;
		    } else if(
		      ((next[smallest][ii + 1] + 12) - next[smallest][ii]) %12
	...
	public int[] getIVector() {
		...
		for (int i = 1; i < set.length; i++)
			for (int ii = 0; ii < i; ii++) {
				int ic = ((set[i] + SCALE_STEPS) - set[ii]) 
					% SCALE_STEPS;
				switch (ic) {
				case 1:
				case 11:
	...
	public int[][] getTns(int min, int max) {
		...
		for (int n = min; n <= max; n++) // n: that of Tn
			for (int j = 0; j < set.length; j++) // j: pitch-class
	...
	public int[][] returnTns(int [] st, int min, int max) {
		...
		for (int n = min; n <= max; n++) // n: that of Tn
			for (int j = 0; j < st.length; j++) // j: pitch class
	...
	public int[][] getTnIs(int min, int max) {
		...
		for(int n = min; n <= max; n++) // n: that of TnI
			for(int j = 0; j < set.length; j++) // j: a pitch-class
				tnis[n - min][j] 
					= (SCALE_STEPS - set[j] + n) 
					% SCALE_STEPS;
		return tnis;
	...
	public int[][] returnTnIs(int [] st, int min, int max) {
		...
		for(int n = min; n <= max; n++) // n: that of TnI
			for(int j = 0; j < st.length; j++) // j: a pitch-class
				tnis[n - min][j] 
					= (SCALE_STEPS - st[j] + n) 
					% SCALE_STEPS;
		return tnis;
	...
	public int[][] getMns(int min, int max) {
		...
		for(int n = min; n <= max; n++) // n: that of Mn
			for(int j = 0; j < set.length; j++) // j: a pitch-class
	...
	public int[][] getRTns(int min, int max) {
		...
		for(int n = min; n <= max; n++) // n: that of RTn
			for(int j = 0; j < set.length; j++) // j: a pitch-class
				rtns[n - min][set.length - 1 - j] 
					= (set[j] + n) % SCALE_STEPS;
		return rtns;
	}
	...
	public int[][] getRTnIs(int min, int max) {
		...
		for(int n = min; n <= max; n++) // n: that of RTnI
			for(int j = 0; j < set.length; j++) // j: a pitch-class
				rtnis[n - min][set.length - 1 - j] 
				      = (SCALE_STEPS - set[j] + n) 
					% SCALE_STEPS;
		return rtnis;
	}
	...
	public int[][] rotateSequence(int min, int max) {
		...
		for(int n = min; n <= max; n++) // n: that of rn
			for(int j = 0; j < set.length; j++) // j: a pitch-class
	...
	public int[] rotateSeqByN(int[] seq, int n) {
		...
		for(int i = 0; i < seq.length; i++) // n: that of rn
